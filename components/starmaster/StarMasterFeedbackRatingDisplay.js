...
const StarMasterFeedbackRatingDisplay = ({ rating }) => {
  ...
  for (let i = 1; i <= maxStars; i++) {
    if (rating >= i) {
      // Full star
      stars.push(
        <View key={`full-${i}`} style={styles.fullStarContainer}>
          <Octicons name="star-fill" size={50} color={Colors.defaultYellow} />
        </View>
      );
...
export default StarMasterFeedbackRatingDisplay;
